...
async function removeResource(context, category, resourceName, questionOptions = {}) {
    ...
    context.print.info('');
    let service = lodash_1.default.get(amplifyMeta, [category, resourceName, 'service']);
    ...
    const confirm = (context.input.options && context.input.options.yes) ||
        (await context.amplify.confirmPrompt('Are you sure you want to delete the resource? This action deletes all files related to this resource from the backend directory.'));
    if (!confirm)
        return;
    try {
