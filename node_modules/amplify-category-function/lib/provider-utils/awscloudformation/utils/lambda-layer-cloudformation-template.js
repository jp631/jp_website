...
function generateLayerCfnObjBase(multiEnvLayer) {
    const cfnObj = {
        ...
        Resources: {},
        Conditions: {
            HasEnvironmentParameter: cloudform_1.Fn.Not(cloudform_1.Fn.Equals(cloudform_1.Fn.Ref('env'), 'NONE')),
        },
    };
    if (multiEnvLayer) {
...
function generateLayerCfnObj(context, parameters) {
    ...
    const outputObj = {
        Outputs: {
            Arn: {
                Value: cloudform_1.Fn.Ref('LambdaLayer'),
            },
            Region: { Value: cloudform_1.Refs.Region },
        },
    };
    let cfnObj = { ...generateLayerCfnObjBase(multiEnvLayer), ...outputObj };
    const POLICY_RETAIN = cloudform_1.DeletionPolicy.Retain;
    const layerName = multiEnvLayer ? cloudform_1.Fn.Sub(`${parameters.layerName}-` + '${env}', { env: cloudform_1.Fn.Ref('env') }) : parameters.layerName;
    const layer = new lambda_1.default.LayerVersion({
        ...
        Content: {
            S3Bucket: cloudform_1.Fn.Ref('deploymentBucketName'),
            S3Key: cloudform_1.Fn.Ref('s3Key'),
        },
        Description: cloudform_1.Fn.Sub('Lambda layer version ${latestVersion}', { latestVersion: cloudform_1.Fn.Ref('layerVersion') }),
        LayerName: layerName,
    });
...
function assignLayerPermissions(layerData, version, layerName, isContentUpdated, multiEnvLayer) {
    ...
    if (versionPermission.isPrivate()) {
        result.push({
            ...
            policy: new lambda_1.default.LayerVersionPermission({
                ...layerVersionPermissionBase,
                Principal: cloudform_1.Refs.AccountId,
            }),
        });
    }
...
function createLayerVersionArn(layerData, layerName, version, isContentUpdated, multiEnvLayer) {
    if (isContentUpdated) {
        if (layerData.getLatestVersion() === Number(version)) {
            return cloudform_1.Fn.Ref('LambdaLayer');
        }
    }
    if (multiEnvLayer) {
        return cloudform_1.Fn.Sub('arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:layer:${layerName}-${env}:${layerVersion}', {
            layerName,
            env: cloudform_1.Fn.Ref('env'),
            layerVersion: version,
        });
    }
    return cloudform_1.Fn.Sub('arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:layer:${layerName}:${layerVersion}', {
        layerName,
        layerVersion: version,
    });
}
